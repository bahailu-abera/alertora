...
def preference_page():
    ...
    if not token:
        ...
        return render_template(
            "preference_form.html", token=None, channels=[], types=[]
        )
    ...
    if not client_doc:
        ...
        return render_template(
            "preference_form.html", token=token, channels=[], types=[]
        )

    allowed_types = [
        t["name"] for t in client_doc.get("notification_types", [])
    ]
    ...
    if request.method == "POST":
        ...
        if not selected_channels or not selected_types:
            ...
            result, status = update_user_preference(
                user_id, client_id, selected_channels, selected_types
            )
            flash(
                "Preferences updated!"
                if status == 200
                else result.get("error", "Update failed")
            )
