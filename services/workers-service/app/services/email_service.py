from app.utils.postgress_utils import log_notification_event, update_notification_status
...
def handle_email_message(message):
    ...
    try:
        send_email_via_gmail(message["user_id"], message["content"], message["user_id"], message["client_id"])
        update_notification_status(log_id, status="sent")
    except Exception as e:
        print(f"[ERROR] SendGrid failed: {e}")
        update_notification_status(log_id, status="retry_pending", increment_retry=True)
