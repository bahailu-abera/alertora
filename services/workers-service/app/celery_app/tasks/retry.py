...
from app.utils.postgress_utils import (
    fetch_retry_pending_notifications,
...
def retry_failed_notifications():
    ...
    failed_logs = fetch_retry_pending_notifications(
        max_retries=MAX_RETRIES, limit=100
    )

    for log in failed_logs:
        print(
            f"[INFO] Retrying log ID: {log.id}, \
                channel: {log.channel}, user: {log.user_id}"
        )
        try:
            # Dispatch based on channel
            if log.channel == "email":
                send_email_via_gmail(
                    log.user_id, log.content, log.user_id, log.client_id
                )
